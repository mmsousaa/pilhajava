class Pilha {
    ...
    push(elemento) {
        if (this.isFull()) {
            alert('Atenção! A pilha atingiu sua capacidade máxima!');
            return false;
        }
        if (elemento.includes(';')) {
            alert('Alerta! O valor contém o caractere!');
            return false;
        }
    ...
    pop() {
        if (this.isEmpty()) {
            alert('Cuidado! A pilha está sem elementos!');
            return false;
        }
        this.elementos.pop();
        return true;
...
class PilhaUI {
    ...
    renderTable() {
        ...
        let retorno = `A quantidade de elementos na pilha é: ${elementos.length}`;
